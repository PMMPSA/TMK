...
class RequestThread extends AsyncTask {
  ...
  public function onCompletion(Server $server) {
    if(isset($this->error)) {
      $server->getPluginManager()->getPlugin("VotePE[PTK]")->getLogger()->error($this->error);
    }
    $server->getPluginManager()->getPlugin("VotePE[PTK]")->rewardPlayer($server->getPlayerExact($this->id), $this->rewards);
    array_splice($server->getPluginManager()->getPlugin("VotePE[PTK]")->queue, array_search($this->id, $server->getPluginManager()->getPlugin("VotePE[PTK]")->queue, true), 1);
  }
