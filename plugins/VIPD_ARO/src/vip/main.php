...
class main extends PluginBase implements Listener{
	...
	public function onPlayerJoin(PlayerJoinEvent $event){
	    ...
	    $date4 = ($t[$n]["day"]-$date3);
		if($date4 < 1){
		    $p->sendMessage('§f•§6§cT§aᗰ§6K§f• §cVip Của Bạn Đã Hết Hạn!');
			$this->vip->remove($n);
			$this->vip->save();
			$this->getLogger()->info(TextFormat::BLUE.'§f•§cT§aᗰ§6K§f•'.$n.' đã hết hạn, xóa vip thành công');
			$this->getServer()->dispatchCommand(new ConsoleCommandSender(),'setgroup '.$n.' Guest');
	        }else{
		        $p->sendMessage('§f•§cT§aᗰ§6K§f• §dVIP bạn còn lại: §a'.$date4.' §dngày');
				$this->getLogger()->info(TextFormat::GREEN.'§f•§cT§aᗰ§6K§f• VIP '.$n.' đã Online, VIP Còn Lại: '.$date4.' ngày');
	        }
		}
	} 
					    ...
					    if($sender->hasPermission('setvip.command.vip')){
							if(!isset($t[$nameX])){
								if($args[2] > 0 && $args[2] < 900){	
									...
									$this->vip->setAll($t);
									$this->vip->save();
									$sender->sendMessage('§f•§cT§aᗰ§6K§f• §aBạn là Vip Trong §e'.$args[2].' §aNgày. Cảm Ơn Bạn Đã Ủng Hộ');
									...
									$sender->sendMessage("§f•§6§cT§aᗰ§6K§f• §cVIP Nhiều Hơn 0 ngày và ít hơn 30 ngày.");
									break;
								}
							}else{
							$sender->sendMessage("§f•§6§cT§aᗰ§6K§f• §cVIP Member này Chưa Hết Hạn, Dùng §a/vip remove §c Để Xóa Vip Trước");
							break;
						}
						}else{
