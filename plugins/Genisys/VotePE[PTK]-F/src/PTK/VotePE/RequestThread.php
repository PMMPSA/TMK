...
class RequestThread extends AsyncTask {
  ...
  public function onCompletion(Server $server) {
    if(isset($this->error)) {
      $server->getPluginManager()->getPlugin("VotePE")->getLogger()->error($this->error);
    }
    $server->getPluginManager()->getPlugin("VotePE")->rewardPlayer($server->getPlayerExact($this->id), $this->rewards);
    array_splice($server->getPluginManager()->getPlugin("VotePE")->queue, array_search($this->id, $server->getPluginManager()->getPlugin("VotePE")->queue, true), 1);
  }
