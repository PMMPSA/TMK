...
class Main extends PluginBase {
      ...
      case "vote":
        if(isset($args[0]) && strtolower($args[0]) == "reload") {
          if(Utils::hasPermission($sender, "votereward.command.reload")) {
            $this->reload();
            $sender->sendMessage("§b[§c❤§eVote-Reward§c❤§b]§a All configurations have been reloaded.");
            break;
          }
        ...
        if(in_array(strtolower($sender->getName()), $this->queue)) {
          $sender->sendMessage("§b[§c❤§eVote-Reward§c❤§b]§a Slow down! We're already checking lists for you.");
          break;
        }
  ...
  public function rewardPlayer($player, $multiplier) {
    ...
    if($multiplier < 1) {
      $player->sendMessage("§b[§c❤§eVote-Reward§c❤§b]§c Bạn Chưa Vote Cho Máy Chủ! Hãy Truy Cập bit.do/tmk-vote Để Vote Cho Máy Chủ!!");
      return;
    }
    $clones = [];
    ...
    $player->sendMessage("§b[§c❤§eVote-Reward§c❤§b]§a You voted on $multiplier server list" . ($multiplier == 1 ? "" : "s") . "!");
  }
