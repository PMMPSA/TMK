...
class Main extends PluginBase implements Listener, CommandExecutor{
	...
	public function onEnable(){
	     $this->Datas = array();
			...
			"wand-item" => 271,
                ...
                $this->prifks = "§b[§d§cT§aᗰ§6K§b-§cWE§b] §e";
	}
	...
	public function setPosition1($player,$position){
		...
		$player->sendMessage($this->prifks."Vị Trí Số 1(".$session["selection"][0][0].", ".$session["selection"][0][1].", ".$session["selection"][0][2].")".$count."[".$this->ID[$player->getName()].":".$this->Meta[$player->getName()]."]");
	...
	public function setPosition2($player,$position){
		...
		$player->sendMessage($this->prifks."Vị Trí Thứ 2(".$session["selection"][1][0].", ".$session["selection"][1][1].", ".$session["selection"][1][2].")".$count."[".$this->ID2[$player->getName()].":".$this->Meta2[$player->getName()]."]");
                $this->sessions[$player->getName()]["selection"] = $session["selection"];
		return true;
	}
		...
		switch($cmd){
			...
			case "wand":
				...
				if($sender->getInventory()->getItem($this->config->get("wand-item"))->getID() === Item::get($this->config->get("wand-item"))->getID()){
					...
					break;
                                        ...
                                        $sender->sendMessage($this->prifks."Đập vào khối thứ 1 để chọn vị trí 1, Chạm vào khối thứ 2 để chọn ví trí 2");
				}
                                return true;
				break;
			case "desel":
			...
			case "limit":
				...
				$sender->sendMessage($this->prifks."The number of blocks limit".($limit === -1 ? "infinite":$limit)." khối đã được đặt thành ".$item->getID);
                                return true;
				break;
			case "pos1":
                                if(!($sender instanceof Player)){
	...
	private function W_set($selection, $blocks, $player){
		...
		$player->sendMessage($this->prifks.$count." khối đã được đặt thành ".$b);
		return true;
	}
